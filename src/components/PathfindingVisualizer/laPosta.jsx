...
export default class PathfindingVisualizer extends Component {
  ...
  visualize(algo) {
    if (!this.state.isRunning) {
      ...
      // camino a dibujar 
      console.log('convertGridToPoints: ', this.convertGridToPoints(grid));
      console.log(`start node: (${startNode.col}, ${startNode.row})`);
      console.log(`end node: (${finishNode.col}, ${finishNode.row})`);

      // const { data: path } = useStarknetCall({
      //   contract: test,
      //   method: "path_finder",
      //   args: [startNode.col, startNode.row, finishNode.col, finishNode.row, this.convertGridToPoints(grid)],
      //   options: { watch: false },
      // });
  ...
  convertGridToPoints = (grid) => {
    const initialGrid = [];
    for (let i = 0; i < grid.length; i++) {
      for (let j = 0; j < grid[i].length; j++) {
        if (this.state.grid[i][j].isWall) {
          // this.state.points.push([i, j]);
          // push 0
          initialGrid.push(1);
        } else {
          // push 1
